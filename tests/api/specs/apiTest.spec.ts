...
test.describe.serial('API test without Authentication', () => {
    let petOrderData: petOrderData = [];

    test('Create pets', async ({request}) => {
        for (let i=0; i<petCount; i++){
            const petData = new createPet();
            const response = await request.post(`${baseUrl}/pet`, { //Step 1: Create new pet
                data: petData
            });
            let responseBody = await response.json();
            expect(responseBody.id).toBe(petData.id); //Step 2: Validate response body
    ...
    test('Create pet order', async ({request}) => {
        for (let i=0; i<petCount; i++){
            ...
            for(let n=0; n<numberOfOrders; n++){
                ...
                const response = await request.post(`${baseUrl}/store/order`, { //Step 1: Create random amount of orders for pet
                    data: orderData
                });
                const responseBody = await response.json();
                expect(responseBody.id).toBe(orderData.id); //Step 2: Validate response body
    ...
    test('Delete created pet orders, validate that order is deleted', async ({request}) => {
        for (let i=0; i<petCount; i++){
            ...
            for(let n=0; n<petOrder.petOrders.length; n++){
                const deleteResponse = await request.delete(`${baseUrl}/store/order/${petOrder.petOrders[n]}`); //Step 1: Delete orders for pet
                const deleteResponseBody = await deleteResponse.json();
                expect(deleteResponseBody.code).toBe(200); //Step 2: Validate response body
                expect(deleteResponseBody.type).toBe('unknown');
                expect(deleteResponseBody.message).toBe(petOrder.petOrders[n].toString());
                const getResponse = await request.get(`${baseUrl}/store/order/${petOrder.petOrders[n]}`); //Step 3: Validate that pet orders are deleted
                const getResponseBody = await getResponse.json();                                         //by getting the deleted order
                expect(getResponseBody.code).toBe(1); //Step 4: Validate response body
...
test.describe.serial('API tests with Authentication' , () => {
    ...
    test('Delete created pets, validate that pets are deleted', async ({request}) => {
        for (let i=0; i<petIds.length; i++){
            ...
            while (attempt < maxRetries && !success){
                try 
                {
                    const response = await request.delete(`${baseUrl}/pet/${petId}`); //Step 1: Delete pet
                    expect(response.status()).toBe(200);
                    const responseBody = await response.json();
                    expect(responseBody.code).toBe(200); //Step 2: Validate response body
